...
//
/*
 * @class Ext.ux.ManagedIFrame Version: 1.2 Author: Doug Hendricks.
 * doug[always-At]theactivegroup.com Copyright 2007-2008, Active Group, Inc. All
 * rights reserved.
 * 
 * ***********************************************************************************
 ...
 * 
 * Usage:<br> <pre><code> // Harnessed from an existing Iframe from markup:
 ...
 * Content loaded in this fashion does not share the same document namespaces as
 * it's parent -- // meaning, there (by default) will be no Ext namespace
 * defined in it since the document is // overwritten after each call to the
 * update method, and no styleSheets. </code></pre> <br> @cfg {Boolean/Object}
 * autoCreate True to auto generate the IFRAME element, or a
 * {@link Ext.DomHelper} config of the IFRAME to create @cfg {String} html Any
 * markup to be applied to the IFRAME's document content when rendered. @cfg
 * {Object} loadMask An {@link Ext.LoadMask} config or true to mask the iframe
 * while using the update or setSrc methods (defaults to false). @cfg {Object}
 * src The src attribute to be assigned to the Iframe after initialization
 * (overrides the autoCreate config src attribute) @constructor @param {Mixed}
 * el, Config object The iframe element or it's id to harness or a valid config
 * object.
 * 
...
(function() {
	...
	Ext.ux.ManagedIFrame = function() {
		...
		if (el && el.dom && el.dom.tagName == 'IFRAME') {
			...
			el = config.autoCreate ? Ext.get(Ext.DomHelper.append(config.autoCreate.parent || document.body, Ext.apply({
				tag: 'iframe',
		...
		el.dom.name || (el.dom.name = el.dom.id); // make sure there is a
													// valid frame name
		el.dom.mifId = el.dom.id; // create an un-protected reference for
									// browsers which prevent access to
									// 'id'(FF3).
		this.addEvents({
			/**
			 * @event focus Fires when the frame gets focus.
			 * @param {Ext.ux.ManagedIFrame}
			 *            this
			 * @param {Ext.Event}
			 *            Note: This event is only available when overwriting
			...
			/**
			 * @event blur * Fires when the frame is blurred (loses focus).
			 * @param {Ext.ux.ManagedIFrame}
			 *            this
			 * @param {Ext.Event}
			 *            Note: This event is only available when overwriting
			...
			/**
			 * @event unload * Fires when(if) the frames window object raises
			 *        the unload event
			 * @param {Ext.ux.ManagedIFrame}
			 *            this
			 * @param {Ext.Event}
			 *            Note: This event is only available when overwriting
			 *            the iframe document using the update method and to
			 *            pages retrieved from a "same domain". Note: Opera does
			 *            not raise this event.
			 */
			"unload": true,

			/**
			 * @event domready Fires ONLY when an iFrame's Document(DOM) has
			 *        reach a state where the DOM may be manipulated (ie same
			 *        domain policy)
			 * @param {Ext.ux.ManagedIFrame}
			 *            this Note: This event is only available when
			 *            overwriting the iframe document using the update
			 *            method and to pages retrieved from a "same domain".
			 *            Returning false from the eventHandler stops further
			 *            event (documentloaded) processing.
			 */
			"domready": true,

			/**
			 * @event documentloaded Fires when the iFrame has reached a
			...
			/**
			 * @event exception Fires when the iFrame raises an error
			 * @param {Ext.ux.ManagedIFrame}
			 *            this
			 * @param {Object/string}
			 *            exception
			...
			"exception": true,
			/**
			 * @event message Fires upon receipt of a message generated by
			 ...
			 * 
			 * @event message:tag Fires upon receipt of a message generated by
			 *        window.sendMessage method which includes a specific tag
			 *        value of the embedded Iframe.window object
			 * @param {Ext.ux.ManagedIFrame}
			 *            this
			 * @param {object}
			 *            message (members: type: {string} literal "message",
		...
		Ext.apply(el, {
			...
			_windowContext: null,
			eventsFollowFrameLinks: typeof config.eventsFollowFrameLinks == 'undefined' ? true : config.eventsFollowFrameLinks
		});
	...
	var MIM = Ext.ux.ManagedIFrame.Manager = function() {
		...
		var implementation = {
			...
			/*
			 * Mask ALL ManagedIframes (eg. when a region-layout.splitter is on
			 * the move.)
			 */
			showShims: function() {
				if (!this.shimApplied) {
	...
	Ext.extend(Ext.ux.ManagedIFrame, Ext.util.Observable, {
		 ...
		 *            without params). Note: invoke the function with no
		 *            arguments to refresh the iframe based on the current src
		 *            value.
		 */
	...
	(function() {
		 ...
		 * 
		 * @param {Element,
		 *            String} node The node id or node Element to remove
		 * 
		 */
		,
		...
		_renderHook: function() {
			...
			this._hooked = false;
			try {
				if (this.writeScript('(function(){(window.hostMIF = parent.Ext.get("' + this.dom.id + '"))._windowContext=' + (Ext.isIE ? 'window' : '{eval:function(s){return eval(s);}}') + ';})();')) {
					...
					EV.addListener(w, 'focus', this._frameProxy);
					EV.addListener(w, 'blur', this._frameProxy);
					EV.addListener(w, 'unload', this._frameProxy);
		 ...
		 * Iframes window object. @param {String} block A valid ('eval'able)
		 * script source block. @param {Boolean} useDOM - if true inserts the fn
		...
		/*
		 * write a <script> block into the iframe's document @param {String}
		 * block A valid (executable) script source block. @param {object}
		 * attributes Additional Script tag attributes to apply to the script
		...
		/*
		 * Eval a function definition into the iframe window context. args:
		 * @param {String/Object} name of the function or function map object:
		 * {name:'encodeHTML',fn:Ext.util.Format.htmlEncode} @param {Boolean}
		 * useDOM - if true inserts the fn into a dynamic script tag, false does
		 * a simple eval on the function definition, examples: var trim =
		...
		showMask: function(msg, msgCls, forced) {
			...
			if ((lmask = this.loadMask) && (!lmask.disabled || forced)) {
				if (lmask._vis)
					return;
				lmask.masker || (lmask.masker = Ext.get(lmask.maskEl || this.dom.parentNode || this.wrap({
					tag: 'div',
					style: {
						position: 'relative'
					}
		...
		hideMask: function(forced) {
			...
			if ((tlm = this.loadMask) && !tlm.disabled && tlm.masker) {
				if (!forced && (tlm.hideOnReady !== true && this._domReady)) {
					return;
				}
				tlm._vis = false;
			...
			switch (rstatus) {
				...
				case 'domready':
					...
					if (this._domReady)
						return;
					this._domReady = true;
					if (this._hooked = this._renderHook()) { // Only raise if
																// sandBox
																// injection
																// succeeded
																// (same domain)
		...
		checkDOM: function(win) {
			...
			var poll = function() { // DOM polling for IE and others
				try {
					...
					n = max; // likely same-origin policy violation, so DOM
								// is ready
				}

				// if the timer has reached 100 (timeout after 3 seconds)
				// in practice, shouldn't take longer than 7 iterations [in kde
				// 3
				// in second place was IE6, which takes 2 or 3 iterations
	...
	/*
	 * @class Ext.ux.ManagedIFramePanel Version: 1.2 (8/22/2008)
	 * 
	 * 
	...
	Ext.ux.ManagedIframePanel = Ext.extend(Ext.Panel, {
		...
		/**
		 * @cfg {String/Object} frameStyle Custom CSS styles to be applied to
		 *      the ux.ManagedIframe element in the format expected by
		...
		autoScroll: false,
		closable: true,
		/* set True by default in the event a site times-out while loadMasked */
		...
		initComponent: function() {
			...
			this.bodyCfg || (this.bodyCfg = {
				cls: 'x-managed-iframe-mask' // shared masking DIV for
												// hosting loadMask/dragMask
				,
				children: [
				    // Ext.apply(
				    Ext.apply({
		...
		onRender: function(ct, position) {
			...
			if (this.iframe = this.body.child('iframe.x-managed-iframe')) {
				...
				if (this.iframe = new Ext.ux.ManagedIFrame(this.iframe, {
					...
					this.relayEvents(this.iframe, ["blur", "focus", "unload", "documentloaded", "domready", "exception", "message"
				...
				while (ownerCt) {
					...
					ownerCt.on('afterlayout', function(container, layout) {
						var MIM = Ext.ux.ManagedIFrame.Manager,
							st = false;
		...
		toggleShim: function() {
			...
			if (this.shim && this.shimCls)
				this.shim.toggleClass(this.shimCls + '-on');
		},
		// private
		...
		on: function(name) {
			...
			if (this.getFrame() && n) {
				...
				if (n)
					this._msgTagHandlers = this._msgTagHandlers.concat(n); // queued
																			// for
																			// onRender
																			// when
																			// iframe
		 ...
		 * @param {Boolean}
		 *            discardUrl (Optional) If not passed as <tt>false</tt>
		 *            the URL of this action becomes the default URL for this
		 *            panel, and will be subsequently used in future setSrc
		 *            calls. Note: invoke the function with no arguments to
		 *            refresh the iframe based on the current defaultSrc value.
		 */
		setSrc: function(url, discardUrl, callback) {
		...
		getState: function() {
			...
			return Ext.apply(Ext.ux.ManagedIframePanel.superclass.getState.call(this) || {}, URI ? {
		 ...
		 * you to perform Ajax-based document replacement of this panel's iframe
		 * document.
		 * 
